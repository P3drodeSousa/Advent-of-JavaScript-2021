...
const asideMenu = document.querySelector("div.wrapper ul#tabs");
...
// RENDER MAIN ON LOAD || UPDATE
function renderMain() {
  const { id, title, description, link } = episodes[selected];
  cover.innerHTML = ` <img src="./images/cover__episode-${id}.png" alt="Episode ${id}" />`;
...
window.addEventListener("load", () => {
  ...
  episodes.forEach((episode, idx) => {
    ...
    liEl.innerHTML = `
          ...
          `;
    asideMenu.append(liEl);

    liEl.addEventListener("click", (e) => {
      asideMenuLinks[selected].classList.remove("selected");
      selected = e.currentTarget.dataset.episode;
      asideMenuLinks[selected].className = "selected";

      renderMain();
    });
  });
  asideMenuLinks = document.querySelectorAll("div.wrapper ul#tabs li");
  asideMenuLinks[selected].className = "selected";
});
